...
- Driven Claude turns and `greatminds daemon doctor` now invoke `claude -p`
  with explicit non-interactive process requirements: `HOME` set to the daemon
  OS user's home, `$HOME/.local/bin` on `PATH`, and an absolute Claude binary
  path when resolvable.
