...
function tasks(){
 ...
 return heading(tr('Задачи проекта'),tr('Очереди и этапы отражают состояние, которое хранит демон.'))+tr`<div class="filter-row"><input id="task-search" class="search" placeholder="Поиск по задачам…" aria-label="Поиск по задачам" value="${esc(state.filter)}"><span class="muted">${count(rows.length,[tr('задача'),tr('задачи'),tr('задач')])}</span></div>`+(groups.length?tr`<div class="queue-board">${groups.map(q=>tr`<section class="queue"><div class="queue-title"><span>${esc(tr(queues[q]||q))}</span><span>${rows.filter(t=>t.queue===q).length}</span></div>${rows.filter(t=>t.queue===q).map(t=>tr`<button class="task-card" data-task="${esc(t.id)}"><small>${esc(t.id)}</small><strong>${esc(t.title || tr('Без названия'))}</strong>${t.error?badge('failed'):''}</button>`).join('')}</section>`).join('')}</div>`:empty(tr('Задачи не найдены'),tr('Начните с диалога с планировщиком или создайте задачу через CLI.')));
}
function runs(){
