...
def build_codex_argv(
    ...
    # 0158: emit ``--profile <role>`` when EITHER the per-role codex
    # home is present at <project>/coordination/.codex-home/<role>/
    # config.toml (0158 mechanism) OR the legacy ~/.codex/<role>.config.toml
    # exists (pre-0158 fallback for projects not yet re-run-setup).
    # ``project_dir`` is derived from registry_dir (= <project>/
    # coordination/.agent_registry).
    codex_profile_args: list[str] = []
    project_dir = registry_dir.parent.parent
    per_role_home = (project_dir / "coordination" / ".codex-home"
                     / role_lower / "config.toml")
    legacy_profile = Path.home() / ".codex" / f"{role_lower}.config.toml"
    if per_role_home.is_file() or legacy_profile.is_file():
...
def start_agent(role: str, tool: str, mode: str,
    ...
    if tool == "claude":
        ...
        # 0158: codex 0.130+ reads its config from $CODEX_HOME/config.toml
        # (selecting [profiles.<role>] via --profile). Point it at the
        # per-role home generated by `greatminds setup` at
        # <project>/coordination/.codex-home/<role>/.
        #
        # 0375 SCOPE NOTE: this is the INTERACTIVE / paned codex launch
        # only. The per-role home is config SOURCE (developer_instructions,
        # model, skills) — it MUST NOT hold an auth.json copy. Driven codex
        # roles (coordd, _codex_appserver_env) deliberately do NOT use this
        # per-role home for auth: they run against the SINGLE machine Codex
        # login ($HOME/.codex), because codex 0.137 refreshes single-use
        # tokens in $CODEX_HOME/auth.json and per-role copies diverge
        # (refresh_token_reused). Do not introduce per-role auth copies /
        # symlinks here.
        #
        # Legacy fallback:
        # if the per-role home doesn't exist (pre-0158 setup not yet
        # re-run), leave CODEX_HOME unset → codex reads ~/.codex/, which
        # under 0.130+ misses the per-role file. Surface a deprecation
        # hint so the operator knows to re-run setup.
        codex_home = project_dir / "coordination" / ".codex-home" / role_lower
        if (codex_home / "config.toml").is_file():
            ...
            if legacy.is_file():
                sys.stderr.write(
                    ...
                    f"`greatminds setup {project_dir}` (idempotent) to "
