...
def _build_settings_local_json(project_dir: Path,
    ...
    settings = {
        # 0310: reverted 0309 — no ``model`` field. Model selection
        # lives in machine-level ~/.claude/settings.json or /model,
...
def _ensure_claude_settings_local(project_dir: Path, canon: Path) -> str:
    ...
    # 0310: 0309's model-injection on merge is reverted. setup does
    # not touch the ``model`` field — operator-written model values
    # in an existing file are left untouched; fresh files don't get
    # one. Model selection is machine-level (~/.claude/settings.json)
    # or interactive (/model).
